...
export function ConfirmDialog({
  ...
  pending?: boolean;
  confirmLabel?: string;
  /** Texto mostrado mientras `pending` es true; por defecto se deriva de confirmLabel. */
  pendingLabel?: string;
  /** "danger" (rojo, borrados) o "success" (verde, ej. marcar como hecha). */
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center px-4 z-30">
      <div
        className={`relative w-full max-w-xs border bg-night-blue rounded-2xl p-5 flex flex-col gap-4 shadow-[0_0_60px_-15px_rgba(255,80,80,0.25)] ${
          isSuccess ? "border-emerald-400/30" : "border-red-400/30"
        }`}
      >
        <CornerBrackets />
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <button
            ...
            onClick={onConfirm}
            disabled={pending}
            className={`border rounded-lg px-3 py-2 text-sm transition disabled:opacity-50 ${
              isSuccess
