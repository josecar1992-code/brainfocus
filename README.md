...
| Filtro Pendientes/Hechas (`StatusFilterTabs.tsx`) | Toggle compartido entre Tareas y Agenda, default "Pendientes" — en Agenda filtra por el status de la tarea vinculada a cada evento (sin tarea vinculada cuenta como pendiente); no aplica a Rutinas (siempre muestra una sola ocurrencia activa por rutina, no una lista completable) |
