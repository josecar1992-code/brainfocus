...
Expone un set chico y de grano grueso a propósito — lista completa a la fecha (20-ago-2026, ver
`apps/mcp/src/index.ts` como fuente de verdad, esta lista tiende a quedarse corta con uso real):
`hora_actual`, `listar_tareas`, `crear_tarea`, `completar_tarea`, `editar_tarea`, `crear_recordatorio`,
`listar_recordatorios`, `editar_recordatorio`, `borrar_recordatorio`, `crear_aviso_asistente`,
`listar_avisos_asistente`, `pausar_aviso_asistente`, `borrar_aviso_asistente`, `crear_evento`,
`crear_nota`, `buscar_notas`, `consultar_consumos`, `listar_vehiculos`, `crear_vehiculo`,
`fijar_proximo_mantenimiento`, `crear_mantenimiento`, `listar_mantenimientos`, `registrar_kilometraje`,
`listar_kilometrajes`, `listar_categorias`, `listar_proyectos`, `crear_proyecto`, `listar_rutinas`,
`crear_rutina`, `guardar_documento`, `buscar_documentos`, `enviar_documento` — cada tool registrado se
inyecta en el prompt del agente en cada turno, así que crece con uso real, no por especulación.
`crear_evento` agenda un evento real (no solo un
...
| Módulo Notas | Nombre + contenido desde la app; Quicks puede escribir (`crear_nota`) y leer/buscar (`buscar_notas`), pero no editar (sin tool `editar_nota` todavía). **Vista resumida** (01-sep-2026): la lista mostraba el contenido completo siempre (sin truncar) con edición inline por ícono de lápiz — ahora muestra un preview truncado (2 líneas, mismo patrón que ya tenían las notas de un proyecto en `ProjectsPage.tsx`) y un click abre la nota completa en el modal `NoteDetail.tsx` (extraído de `ProjectsPage.tsx` a componente compartido, con ver/editar/borrar) |
| Recordatorios ↔ cron de OpenClaw | Automático de punta a punta y verificado en producción (crear, reprogramar si cambia la fecha, cancelar al completar/borrar tarea o evento). Canal default **`whatsapp`** (07-ago-2026, vía integración Kapso — API oficial de Meta, sin las desconexiones de sesión de la integración anterior con Baileys; confirmado con test de entrega en vivo). `reminders.channel` sigue guardando `"whatsapp"`/`"telegram"`/`"email"` como siempre; `scheduleReminderCron` (`apps/api/src/services/openclawCron.ts`) es la única capa que traduce `"whatsapp"` → `"kapso-whatsapp"`, el identificador que espera el gateway de OpenClaw tras esa migración. Telegram sigue disponible como canal alterno (usa un chat ID numérico dedicado, `OPENCLAW_REMINDER_TO_TELEGRAM`). **Regla fija al tocar el canal default**: desplegar primero el código, migrar la DB después (nunca al revés — un desfase entre ambos causó un 500 real en `crear_recordatorio` el mismo 07-ago, ver el historial en [infra/DEPLOY.md](infra/DEPLOY.md)), y revisar todos los cron jobs pendientes con el canal viejo. **Guarda anti-duplicado** (08-ago-2026): `POST /reminders` rechaza (409) un recordatorio con mismo título+hora si ya existe uno creado hace menos de 2 minutos — el modelo de Quicks a veces llama la misma tool dos veces en el mismo turno (confirmado: "Tomar la proteína" x2 con 11s de diferencia, dos POST reales, no un retry de red), ver `beforeCreate` en `apps/api/src/routes/reminders.ts`. **Respaldo por Telegram** (10-ago-2026, pedido explícito del usuario; rediseñado 02-sep-2026): WhatsApp/Kapso puede fallar en silencio (ventana de 24h cerrada, u otras fallas de entrega) y, para jobs `payload.kind:"agentTurn"` (los que usan los recordatorios), **el propio gateway de OpenClaw downgradea a propósito el status del job de vuelta a "ok" cuando la entrega falla pero el turno del agente no** — confirmado leyendo el fuente instalado en el VPS (`isolated-agent-*.js`). Por diseño del gateway no existe forma de detectar la falla desde afuera para reaccionar a ella (tampoco sirve `reminders.sent_at`, que nunca se llega a llenar en la práctica). Por eso el mecanismo ya no es un "fallback ante fallo" sino un **respaldo incondicional**: `scheduleReminderBackupCron` (`openclawCron.ts`) programa un segundo cron, siempre, `BACKUP_DELAY_MINUTES = 4` minutos después del principal, que manda el mismo recordatorio por Telegram sin importar si el de WhatsApp llegó o no — mismo patrón que la automatización de n8n del Poder Judicial (dos canales en paralelo). Se omite solo si el canal principal ya es Telegram. El job id del respaldo vive en `reminders.backup_cron_job_id` (columna nueva), cancelado/reprogramado junto con el principal en los mismos puntos (`reminders.ts`, `reminderCascade.ts`). El intento anterior (`delivery.failureDestination`, corregido 01-sep-2026 tras un primer bug de nombre de campo) quedó documentado en [PENDIENTES.md](PENDIENTES.md) como vía muerta para este tipo de job — no se retiró el código (sigue siendo correcto para otros tipos de job/gateway), solo dejó de ser la única defensa. |
...
| Sidebar | Orden (16-ago-2026): Hoy, Agenda, Tareas, Asistente, Proyectos, Rutinas, Notas y memorias, Documentos, Vehículos, Consumos, Resumen, Configuración — Hoy es el módulo por defecto al iniciar (antes era Agenda). En móvil, el drawer se abre desde la izquierda, junto al botón de hamburguesa |
| Vista Hoy | `TodayPage.tsx` (09-ago-2026): consolida eventos de hoy + tareas que vencen hoy + la ocurrencia de rutina de hoy en una sola pantalla, con el mismo checkbox de completar (`useCompleteTask`) que Tareas/Agenda/Rutinas — no agrega datos nuevos, solo une lo que ya existía en tres módulos distintos. Orden de secciones (16-ago-2026): eventos de hoy → tareas que vencen hoy → tareas atrasadas → rutinas de hoy → **sin fecha límite** (23-ago-2026, sección nueva al final: tareas pendientes sin `due_date` no aparecían en ninguna sección de Hoy y quedaban totalmente fuera de vista, solo visibles entrando a Tareas — pedido por el usuario para que no se pierdan) |
| Módulo de Proyectos | `ProjectsPage.tsx` (09-ago-2026): agrupa tareas/eventos/notas/documentos bajo un proyecto (`project_id` nullable en esas 4 tablas), a diferencia de `lists`/categorías que solo aplica a tareas. Progreso agregado (verde) calculado en el cliente a partir de las tareas ligadas, mismo patrón que subtareas. Crear/archivar/reactivar/borrar (borrar no borra lo ligado, solo lo deja sin proyecto). Selector de proyecto wireado en los formularios de Tareas/Agenda/Notas/Documentos. Quicks: `crear_proyecto`, `listar_proyectos`, `proyecto_id` opcional en crear_tarea/crear_evento/crear_nota/guardar_documento. **Notas y documentos clickeables** (20-ago-2026): antes las notas del proyecto solo mostraban un preview truncado sin interacción, y los documentos ligados no aparecían ahí para nada (había que buscarlos en el módulo Documentos aparte). Ahora una nota abre un modal (`NoteDetail`, con scroll para notas largas) con ver/editar/borrar completo, y una sección "Documentos" nueva lista los archivos del proyecto — click abre el archivo en pestaña nueva vía `getDocumentDownloadUrl` |
...
| Módulo Resumen | (16-ago-2026) `ResumenPage.tsx` — tareas marcadas como hechas en un día/rango (filtros rápidos Hoy/Ayer/Últimos 7 días + rango manual), día calendario de Costa Rica con offset explícito. Reusa `GET /tasks` con `status=done` + `completed_at_gte`/`completed_at_lte` (filtro genérico `_gte`/`_lte` de `resourceRouter.ts`, sin endpoint nuevo) y el `TaskDetail` compartido de Tareas al hacer click. De paso corrigió que `tasks.completed_at` (columna que ya existía) nunca se llenaba al completar una tarea desde el checkbox de la web — ahora la llena/limpia sola la API (`beforeUpdate`, hook nuevo en `resourceRouter.ts`) al cruzar el status "done", sin importar quién mande el `PATCH` |
