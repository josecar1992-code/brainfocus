...
| Recordatorios ↔ cron de OpenClaw | Automático de punta a punta y verificado en producción (crear, reprogramar si cambia la fecha, cancelar al completar/borrar tarea o evento). Canal default sigue en **`telegram`** (usa un chat ID numérico dedicado, `OPENCLAW_REMINDER_TO_TELEGRAM`, ya que ese canal no acepta teléfonos como destinatario). Se intentó revertir a `whatsapp` el 07-ago-2026 tras migrar la integración a Kapso (API oficial de Meta, `scheduleReminderCron` ya traduce `"whatsapp"` → `"kapso-whatsapp"` para el gateway), pero un test de entrega en vivo no llegó — pendiente de diagnóstico de configuración en OpenClaw/Kapso (`dm:allowlist`, ver [infra/DEPLOY.md](infra/DEPLOY.md)) antes de reintentar. **Regla fija al tocar el canal default**: revisar todos los cron jobs pendientes con el canal viejo, ver la regla en [infra/DEPLOY.md](infra/DEPLOY.md) |
