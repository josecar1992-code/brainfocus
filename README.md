...
| Sidebar | Orden (09-ago-2026): Hoy, Agenda, Tareas, Proyectos, Rutinas, Notas y memorias, Documentos, Vehículos, Configuración — Hoy es el módulo por defecto al iniciar (antes era Agenda). En móvil, el drawer se abre desde la izquierda, junto al botón de hamburguesa |
...
| Recordatorios recurrentes | `recurring_reminders` (09-ago-2026): "recordame tomar agua cada 2 horas" / "todos los días a las 8pm" / "los lunes a las 9am", sin crear una rutina completa (no genera tarea ni queda en ningún historial). Reusa el mismo cron **recurrente** real de OpenClaw que el aviso mensual de kilometraje (`scheduleRecurringCron`); se puede pausar sin borrar. Configurable desde Configuración → "Avisos recurrentes". Quicks: `crear_recordatorio_recurrente`, `listar_recordatorios_recurrentes`, `pausar_recordatorio_recurrente`, `borrar_recordatorio_recurrente` |
